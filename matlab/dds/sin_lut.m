...
angle_step = fi(2*pi / num_entries, 1, 16, 12);

% Initialize an empty array to store the sine values in fixed-point format
sin_lut_fixed = fi(zeros(1, num_entries), 1, 16, 8);

% Generate the sine LUT in fixed-point format
for i = 1:num_entries
    angle = fi((i - 1) * angle_step, 1, 16, 8);
    sine_value = fi(sin(angle), 1, 16, 8); % Store as signed 16.8 fixed-point
    sin_lut_fixed(i) = sine_value;
end

%Create a file for writing
fileID = fopen('D:\Digital Electronics\DSP\DSP_course\rtl\dds\sin_lut_init_file.txt', 'w');
